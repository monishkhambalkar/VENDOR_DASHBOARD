...
const SignIn: React.FC = () => {
  ...
  //   setEmailValidation(formData.sEmail == "" ? true : false);
  //   setPasswordValidation(formData.sPassword == "" ? true : false);

  //   setLoginValidation(true);

  //   if (!formData.sEmail || !formData.sPassword) return;

  //   try {
  //     // const apiUrl = `${config.VENDOR_BASE_URL}/user/login`;
  ...
  // Hide login alert
  const handleHideLoginAlert = (event: React.MouseEvent<HTMLButtonElement>) => {
    event.preventDefault();
    setLoginValidation(false);
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const isEmailEmpty = formData.sEmail.trim() === "";
    const isPasswordEmpty = formData.sPassword.trim() === "";

    setEmailValidation(isEmailEmpty);
    setPasswordValidation(isPasswordEmpty);
    setLoginValidation(true);
