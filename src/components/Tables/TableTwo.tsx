...
const TableTwo = () => {
  ...
  useEffect(() => {
    ...
    fetchProducts();
  }, [productSearch, deleteProductSuccess, currentProductPage]);
  ...
  const handleDeleteProduct = async (productID) => {
    ...
    try {
      const deleteProduct = await axios.delete(
        `${config.VENDOR_BASE_URL}/product/delete-product/${productID}`
      );
      if (deleteProduct.status == 200) {
        setDeleteProductSuccess(true);
        alertNotificationFunction("success", "Your Product Deleted");
      } else {
        alertNotificationFunction(
          "error",
          "Something went Wong, Product Not deleted"
        );
      }
    } catch (error) {
    ...
    <>
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="py-6 px-4 md:px-6 xl:px-7.5 flex justify-between items-center">
          ...
          <ProductBulkUploadModal isOpen={isModalOpen} onClose={closeModal} />
        </div>
        ...
        <div className="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
          ...
          <div className="col-span-2 hidden items-center sm:flex">
            <p className="font-medium">Available Qty</p>
          </div>
          <div className="col-span-1 flex items-center">
